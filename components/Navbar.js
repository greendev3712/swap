...
const Navbar = () => {
	return (
		<div className="w-[95%] rounded-3xl h-20 bg-[#f6f6f7] text-center flex justify-center items-center fixed top-5 z-10 shadow-[1px_1px_17px_1px]">
			{/* logo container */}
				...
				{headerItems.map((item, index) => {
					return (
						<Link href={`/${item.toLowerCase()}`} key={index}>
							<a className="text-xl text-[#242424] underline underline-offset-8 underline-color decoration-[#90E040]">
								{item}
							</a>
						</Link>
					);
